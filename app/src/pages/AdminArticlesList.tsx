...
export default function AdminArticlesList() {
  ...
  useEffect(() => {
    ...
    authenticatedFetch(apiUrl('/api/articles'), {}, token)
      ...
      .catch(() => { if (!cancelled) setError('Could not load articles'); })
    ...
    <div className="min-h-screen bg-forest px-6 py-12">
      <div className="max-w-3xl mx-auto">
        <div className="flex items-center justify-between gap-4 mb-8">
          ...
          <Link to="/admin" state={{ openTab: 'articles' }} className="px-3 py-2 bg-lime text-forest text-sm font-medium hover:bg-lime/90 transition-colors">
            Add new article
          </Link>
        </div>
        <h1 className="text-offwhite font-editorial font-bold text-2xl mb-2">Articles</h1>
        <p className="text-offwhite/60 text-sm mb-6">Manage and edit existing articles.</p>
        {error && <p className="text-red-400 text-sm mb-4">{error}</p>}
        {loading ? (
          <p className="text-offwhite/60">Loading…</p>
        ) : list.length === 0 ? (
          <p className="text-offwhite/60">No articles yet. <Link to="/admin" state={{ openTab: 'articles' }} className="text-lime hover:underline">Add one</Link>.</p>
        ) : (
          <>
          <ul className="space-y-2">
            {pageList.map((a) => (
              <li key={a.id} className="flex items-center justify-between gap-4 py-3 border-b border-offwhite/10">
                <span className="text-offwhite truncate flex-1">{a.title}</span>
                <div className="flex items-center gap-2 shrink-0">
                  <button
                    type="button"
                    onClick={() => navigate('/admin', { state: { openTab: 'articles', editId: a.id } })}
                    className="p-2 text-offwhite/70 hover:text-lime transition-colors"
                    title="Edit"
                  >
                    <Pencil className="w-4 h-4" />
                  </button>
                  <button type="button" onClick={() => handleDelete(a.id)} className="p-2 text-offwhite/70 hover:text-red-400 transition-colors" title="Delete">
            ...
            <div className="mt-6 flex flex-wrap items-center justify-between gap-4 border-t border-offwhite/10 pt-4">
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  disabled={safePage <= 1}
                  className="p-2 text-offwhite/70 hover:text-lime disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
