...
export default function AdminTeamList() {
  ...
  useEffect(() => {
    ...
    fetch(apiUrl('/api/team'), { headers: { Authorization: `Bearer ${token}` } })
      ...
      .catch(() => { if (!cancelled) setError('Could not load team'); })
    ...
    <div className="min-h-screen bg-forest px-6 py-12">
      <div className="max-w-3xl mx-auto">
        <div className="flex items-center justify-between gap-4 mb-8">
          ...
          <Link to="/admin" state={{ openTab: 'team' }} className="px-3 py-2 bg-lime text-forest text-sm font-medium hover:bg-lime/90 transition-colors">
            Add team member
          </Link>
        </div>
        <h1 className="text-offwhite font-editorial font-bold text-2xl mb-2">Team</h1>
        <p className="text-offwhite/60 text-sm mb-6">Manage and edit existing team members.</p>
        {error && <p className="text-red-400 text-sm mb-4">{error}</p>}
        {loading ? (
          <p className="text-offwhite/60">Loading…</p>
        ) : list.length === 0 ? (
          <p className="text-offwhite/60">No team members yet. <Link to="/admin" state={{ openTab: 'team' }} className="text-lime hover:underline">Add one</Link>.</p>
        ) : (
          <>
          <ul className="space-y-2">
            {pageList.map((t) => (
              <li key={t.id} className="flex items-center justify-between gap-4 py-3 border-b border-offwhite/10">
                ...
                <div className="flex items-center gap-2 shrink-0">
                  <button
                    type="button"
                    onClick={() => navigate('/admin', { state: { openTab: 'team', editId: t.id } })}
                    className="p-2 text-offwhite/70 hover:text-lime transition-colors"
                    title="Edit"
                  >
                    <Pencil className="w-4 h-4" />
                  </button>
                  <button type="button" onClick={() => handleDelete(t.id)} className="p-2 text-offwhite/70 hover:text-red-400 transition-colors" title="Delete">
